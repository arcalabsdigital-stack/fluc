...
//     USING: ((id IN ( SELECT user_workspaces.organization_id    FROM user_workspaces   WHERE (user_workspaces.user_id = auth.uid()))) OR (id = get_current_user_org_id()))
// Table: parcelated_transactions
...
//     USING: (is_admin() AND (organization_id IN ( SELECT user_workspaces.organization_id    FROM user_workspaces   WHERE ((user_workspaces.user_id = auth.uid()) AND (user_workspaces.role = 'admin'::text)))))
...
//     USING: ((id = auth.uid()) OR (organization_id IN ( SELECT user_workspaces.organization_id    FROM user_workspaces   WHERE (user_workspaces.user_id = auth.uid()))))
// Table: recurring_transactions
...
//     USING: ((user_id = auth.uid()) OR (organization_id IN ( SELECT user_workspaces_1.organization_id    FROM user_workspaces user_workspaces_1   WHERE (user_workspaces_1.user_id = auth.uid()))))
