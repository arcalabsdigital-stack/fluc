...
//     USING: (EXISTS ( SELECT 1    FROM user_workspaces uw   WHERE ((uw.user_id = profiles.id) AND (uw.organization_id = get_current_user_org_id()))))
// Table: recurring_transactions
...
//     USING: ((auth.uid() = user_id) OR (organization_id IN ( SELECT user_workspaces_1.organization_id    FROM user_workspaces user_workspaces_1   WHERE (user_workspaces_1.user_id = auth.uid()))))
