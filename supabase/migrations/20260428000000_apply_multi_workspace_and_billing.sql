...
SELECT id, 'Fluxo', 'trial', now(), now() + interval '7 days' FROM public.organizations
...
CREATE POLICY "Users can view their own organization" ON public.organizations
  FOR SELECT TO authenticated USING (
    id IN (SELECT organization_id FROM public.user_workspaces WHERE user_id = auth.uid())
...
DROP POLICY IF EXISTS "Users can update subscriptions in their org" ON public.subscriptions;
CREATE POLICY "Users can update subscriptions in their org" ON public.subscriptions
...
DROP POLICY IF EXISTS "Users can insert billing in their org" ON public.billing_history;
CREATE POLICY "Users can insert billing in their org" ON public.billing_history
