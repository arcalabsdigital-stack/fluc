...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!cupom || typeof cupom !== 'string' || !cupom.trim()) {
      return new Response(
        JSON.stringify({ valido: false, erro: 'Cupom inválido' }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        },
    ...
    const fetchAsaas = async (
      ...
      try {
        const response = await fetch(
          'https://sandbox.asaas.com/api/v3/coupons/validate',
          {
            method: 'POST',
            headers: {
    ...
    if (ASAAS_API_KEY === 'dummy') {
      ...
      if (cleanCupom === 'INVALIDO') {
        return new Response(
          JSON.stringify({ valido: false, erro: 'Cupom não encontrado' }),
          {
            status: 404,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          },
      ...
      if (cleanCupom === 'EXPIRADO') {
        return new Response(
          JSON.stringify({ valido: false, erro: 'Cupom expirado' }),
          {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          },
    ...
    if (response.status === 400) {
      return new Response(
        JSON.stringify({ valido: false, erro: 'Cupom inválido ou expirado' }),
        {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        },
    ...
    return new Response(
      JSON.stringify({
        valido: false,
        erro: 'Erro interno ao validar cupom. Tente novamente mais tarde.',
      }),
      {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
